...
namespace libcron
{
	...
	class Cron
	{
	 public:
		...
		bool change_schedule(const std::string& name, const std::string& schedule);
	...
	template<typename ClockType, typename LockType>
	bool Cron<ClockType, LockType>::change_schedule(const std::string& name, const std::string& schedule)
	{
		auto cron = CronData::create(schedule);
		bool res = cron.is_valid();
		if (res)
		{
			...
			tasks.remove(task);
			task.reschedule(CronSchedule{ cron });
			if (task.calculate_next(clock.now()))
			{
				tasks.push(task);
				tasks.sort();
			}
