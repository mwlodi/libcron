...
namespace libcron
{
	...
	class Task : public TaskInformation
	{
	 public:
		...
		void reschedule(const CronSchedule schedule)
		{
			this->schedule = schedule;
			delay = std::chrono::seconds(-1);
			last_run = std::numeric_limits<std::chrono::system_clock::time_point>::min();
			next_schedule = std::chrono::system_clock::time_point::min();
		}
